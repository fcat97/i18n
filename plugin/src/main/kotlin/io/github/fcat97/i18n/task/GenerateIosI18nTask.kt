...
import io.github.fcat97.i18n.generator.IosStringsGenerator
...
abstract class GenerateIosI18nTask : DefaultTask() {
    @get:Input
    ...
    fun generate() {
        ...
        val output = if (outputDir.isPresent) {
            ...
            project.file(I18nExtension.DEFAULT_IOS_OUTPUT_DIR)
        }
        val credentials = credentialsFile.asFile.orNull
        val cacheDir = getCacheDir()

        logger.quiet("[i18n] Starting iOS localization generation")
        ...
        try {
            ...
            logger.quiet("[i18n] Generating iOS .strings resources...")
            IosStringsGenerator.generate(localizationData, output)
            ...
            logger.quiet("[i18n] iOS localization generation complete!")
        } catch (e: Exception) {
